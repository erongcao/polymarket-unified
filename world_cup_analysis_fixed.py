...
# 模拟价格-结果因果关系
price_series = np.array([0.120, 0.135, 0.142, 0.155, 0.148, 0.152])
outcome_series = np.array([0.115, 0.118, 0.122, 0.128, 0.125, 0.120])

# 计算影响系数 L_f
price_diff = np.diff(price_series)
outcome_diff = np.diff(outcome_series)

if len(price_diff) > 1 and len(outcome_diff) > 1:
    correlation = np.corrcoef(price_diff, outcome_diff)[0,1]
    L_f = abs(correlation) * 0.5 if not np.isnan(correlation) else 0.2
else:
    L_f = 0.2  # 默认值
...
argentina_bias = world_cup['outcomes'][0]['prob'] - world_cup['outcomes'][0]['true_prob']
print(f"  • 阿根廷略有高估({argentina_bias:+.1%})，可能受卫冕冠军光环影响")
print(f"  • 法国和巴西定价相对准确")
print()
print("💰 流动性评估:")
...
print()
print("🎯 价值投资机会:")
germany_bias = world_cup['outcomes'][3]['true_prob'] - world_cup['outcomes'][3]['prob']
england_bias = world_cup['outcomes'][5]['true_prob'] - world_cup['outcomes'][5]['prob']
print(f"  • 德国低估{germany_bias:+.1%} - 重建完成可能黑马")
print(f"  • 英格兰低估{england_bias:+.1%} - 凯恩决赛年")
print()
print("⚠️ 风险提示:")
